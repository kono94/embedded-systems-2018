...
bool setInstructionsForRow(uint8_t row){
    switch (row){
        ...
        case 2:
            ...
            return true;
        case 3:
            memcpy(display_data,    getInstructionFromWeekdayIndex(p_avrDatetime->weekdayIndex), 2 * font_width);

            display_toSend_currentSession = 12;
            display_toSend = 12;
            return true;
        case 4:
            memcpy(display_data,    getInstructionFromCurrentDCFPos(), 4 * font_width);
            display_toSend_currentSession = 24;
            display_toSend = 24;
            return true;
        case 5:
            memcpy(display_data,    getInstructionForCurrentDCFStatus(), 7 * font_width);
